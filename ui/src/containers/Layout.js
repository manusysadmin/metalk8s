...
const Layout = () => {
  ...
  // Remove the access to dashboard page if we don't enable/specify in feature gate(config.json - flags).
  if (api && api.flags && !api.flags.includes('dashboard')) {
    ...
    <ThemeProvider theme={theme}>
      <CoreUILayout sidebar={isUserLoaded && sidebarConfig} navbar={navbar}>
        ...
        <Switch>
          ...
          {api && api.flags.includes('dashboard') && (
            <PrivateRoute exact path="/" component={DashboardPage} />
          )}
          <PrivateRoute exact path="/alerts" component={ClusterMonitoring} />
          <PrivateRoute
            exact
